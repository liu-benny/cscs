...
<form method="post" action="<?= URLROOT; ?>/Personnel/change_location/<?= $data['personnel']->personnel_id ?>">
    <h3>Change Personnel work location for <?= $data['personnel']->first_name ?> <?= $data['personnel']->last_name ?></h3>
    ...
    <button type="button" class="btn btn-secondary" onclick="window.location.href='<?= URLROOT; ?>/Personnel/edit_personnel/<?= $data['personnel']->personnel_id ?>'">Back</button>
